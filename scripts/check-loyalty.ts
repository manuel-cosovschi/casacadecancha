...
check('2 compras -> 15%',           percentForOrders(2), 15);
check('3 compras -> 20%',           percentForOrders(3), 20);
check('5 compras -> 20% (tope)',    percentForOrders(5), 20);
check('12 compras-> 20% (tope)',    percentForOrders(12), 20);

console.log('\n--- lo que ve el cliente ---');
check('con 0: le falta 1 para el 10%', statusFromOrders(0), {orders:0,percent:0,toNext:1,nextPercent:10});
check('con 1: le falta 1 para el 15%', statusFromOrders(1), {orders:1,percent:10,toNext:1,nextPercent:15});
check('con 2: le falta 1 para el 20%', statusFromOrders(2), {orders:2,percent:15,toNext:1,nextPercent:20});
check('con 3: ya está al tope',        statusFromOrders(3), {orders:3,percent:20,toNext:null,nextPercent:null});
...
check('NO vale para otro nivel', verifyLoyaltyCode('ana@test.com', 20, ana15, hoy), false);
check('al subir de nivel cambia el código', loyaltyCode('ana@test.com', 20, hoy) !== ana15, true);
...
check('umbral configurado', LOYALTY.minOrderAmount, 100000);
